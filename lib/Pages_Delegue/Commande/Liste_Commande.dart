...
import 'package:gestion_delegue/Pages_Delegue/Read_File.dart';
...
class Liste_Commande extends StatelessWidget{
  ...
  Liste_Commande({this.nbreRapport=" ",this.nbreVoyage=" ",this.nbreRendez=" ",this.nbreCommande=" ",this.Nom,this.Prenom,this.Email});

  Future<List> getData() async{
    final response= await http.post("http://192.168.1.67:80/Delegue/Liste_commande.php",body: {
      "email":Email,
    });
          ...
          return new Card(
            ...
            child: new ListTile(
              ...
              leading: ConstrainedBox(
                ...
                child: Image.asset("assets/images/Commande.png", fit: BoxFit.cover),
              ),
              title:Text("Destinateur: "+list[i]['nom']+" "+list[i]['prenom'] , style: TextStyle(color: Colors.blue),),
              subtitle: Text("Date: "+list[i]['date']+"\nAdresse: "+list[i]['adresse'],),
                onTap: (){
                  Navigator.of(context).push(
                    MaterialPageRoute(builder: (BuildContext context)=> ReadFile(pdfasset:"assets/"+list[i]['path'],titre:"Commande")),
                  );
