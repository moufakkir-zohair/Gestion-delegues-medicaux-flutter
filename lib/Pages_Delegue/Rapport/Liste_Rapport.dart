...
class Liste_Rapport extends StatelessWidget{
  ...
  Liste_Rapport({this.nbreRapport=" ",this.nbreVoyage=" ",this.nbreRendez=" ",this.nbreCommande=" ",this.Nom,this.Prenom,this.Email});

  Future<List> getData() async{
    final response= await http.post("http://192.168.1.67:80/Delegue/Liste_rapport.php",body:{
      "email":Email,
    });
        ...
        itemBuilder: ( ctx,i) {
          return new Card(
            ...
            child: new ListTile(
                ...
                title:Text("Sujet: "+list[i]['Sujet'], style: TextStyle(color: Colors.blue),),
                subtitle: Text("Date: "+list[i]['Date_creation'],),
              onTap: (){
                Navigator.of(context).push(
                  MaterialPageRoute(builder: (BuildContext context)=> ReadFile(pdfasset:"assets/"+list[i]['path'],titre:"Rapport")),
                );
              },
